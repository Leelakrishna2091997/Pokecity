...
import { PlayerLocation, Pokemon } from '../../types/CoveyTownSocket';
...
import GrassArea from './interactables/GrassArea';
import PokemonCaptureScreen from './interactables/Pokemon/PokemonCaptureScreen';
import eventsCenter from './interactables/Pokemon/EventsCenter';
import { PlayerID } from '../../generated/client';
import { getDisplayedUserPokemon } from '../../utils/PokemonStorage';
...
export default class TownGameScene extends Phaser.Scene {
  ...
  private _pokemon: Map<PlayerID, TownPokemon> = new Map();
  ...
  private _captureScene?: Phaser.Scene;
  ...
  preload() {
    this.updatePokemon(this.coveyTownController.ourPlayer, { loadLocally: true });
    ...
    this.load.image('Grass_Area', this._resourcePathPrefix + '/assets/tilesets/Grass_Area.png');
  ...
  updatePlayers(players: PlayerController[]) {
    ...
    disconnectedPlayers.forEach(disconnectedPlayer => {
      if (disconnectedPlayer.gameObjects) {
        ...
        const pokemon = this._pokemon.get(disconnectedPlayer.id);
        if (pokemon) {
          pokemon.image?.destroy();
          pokemon.tween?.destroy();
          this._pokemon.delete(disconnectedPlayer.id);
        }
      }
    });
  ...
  movePokemon(player: PlayerController) {
    const townPokemon = this._pokemon.get(player.id);

    if (!townPokemon?.image || !player.gameObjects?.sprite) return;
    const body = townPokemon.image.body as Phaser.Physics.Arcade.Body;
    if (!body) return;

    this.physics.moveToObject(townPokemon.image, player.gameObjects?.sprite, 120, 700);

    if (body.velocity.y >= 0) {
      townPokemon.image.flipX = body.velocity.x >= 0;
      townPokemon.image.setTexture('pokemon-front' + player.id);
    } else {
      townPokemon.image.flipX = body.velocity.x < 0;
      townPokemon.image.setTexture('pokemon-back' + player.id);
    }

    if (Math.abs(body.velocity.x) < 15 && Math.abs(body.velocity.y) < 15) {
      townPokemon.tween?.pause();
    } else {
      townPokemon.tween?.resume();
    }
  }

  async updatePokemon(player: PlayerController, options?: { loadLocally?: boolean }) {
    const pokemonFront = 'pokemon-front' + player.id;
    const pokemonBack = 'pokemon-back' + player.id;

    const townPokemon = this._pokemon.get(player.id);
    // remove pokemon textures if they already exist
    townPokemon?.image?.destroy();
    townPokemon?.tween?.destroy();
    if (this.textures.exists(pokemonFront)) this.textures.remove(pokemonFront);
    if (this.textures.exists(pokemonBack)) this.textures.remove(pokemonBack);

    const pokemon = options?.loadLocally ? getDisplayedUserPokemon() : player._pokemon;
    if (options?.loadLocally) this.coveyTownController.emitDisplayPokemonUpdate(pokemon);

    if (!pokemon) return;

    this.load.image(pokemonFront, pokemon.sprite.front);
    this.load.image(pokemonBack, pokemon.sprite.back);
    ...
    const newTownPokemon: TownPokemon = { pokemon };

    newTownPokemon.image = this.add
      .image(player.location.x, player.location.y, pokemonFront)
      .setDepth(9);
    this.physics.add.existing(newTownPokemon.image);
    newTownPokemon.tween = this.tweens.add({
      targets: newTownPokemon.image,
      duration: 200,
      yoyo: true,
      repeat: -1,
      angle: { from: -10, to: 10 },
    });
    this._pokemon.set(player.id, newTownPokemon);
  }

  getNewMovementDirection() {
    if (this._captureScene?.scene.isVisible()) {
      return undefined;
    }
  ...
  create() {
    ...
    this._players.forEach(player => {
      if (player.id === this.coveyTownController.ourPlayer.id) return;
      return this.updatePokemon(player);
    });
    ...
    this.coveyTownController.addListener('updatePokemon', player => {
      if (player.id === this.coveyTownController.ourPlayer.id) return;
      return this.updatePokemon(player);
    });

    this._captureScene = this.scene.add('pokemonCapture', PokemonCaptureScreen, true) as
      | Phaser.Scene
      | undefined;
    assert(this._captureScene);
    this._captureScene?.scene.setVisible(false);

    eventsCenter.off('reload-pokemon');
    eventsCenter.on('reload-pokemon', () =>
      this.updatePokemon(this.coveyTownController.ourPlayer, { loadLocally: true }),
    );

    //call socket event change

    this.events.on(Phaser.Scenes.Events.SHUTDOWN, () => {
      eventsCenter.off('reload-pokemon');
      this.coveyTownController.removeAllListeners();
    });
  }
