...
export default class Town {
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    // Register an event listener for the client socket: if the client updates their
    // pokemon, inform the CoveyTownController
    socket.on('pokemonDisplay', (newPokemon: Pokemon | undefined) => {
      this._updatePokemonDisplayed(newPlayer, newPokemon);
    });
  ...
  private _updatePokemonDisplayed(player: Player, pokemon: Pokemon | undefined): void {
    // might have to deal with interactables?

    player.pokemon = pokemon;
    this._broadcastEmitter.emit('updatePokemon', player.toPlayerModel());
  }
