...
export type TownEvents = {
  displayPokemon: (pokemon: Pokemon | undefined) => void;
  /**
   *
   * @param pokemon
   * @returns
   */
  updatePokemon: (targetPlayer: PlayerController) => void;
  /**
...
export default class TownController extends (EventEmitter as new () => TypedEmitter<TownEvents>) {
  ...
  registerSocketListeners() {
    ...
    this._socket.on('playerJoined', newPlayer => {
      ...
      if (newPlayerObj.id !== this._ourPlayer?.id) {
        this.emitDisplayPokemonUpdate(getDisplayedUserPokemon());
      }
    });
    /**
    ...
    this._socket.on('updatePokemon', targetPlayer => {
      const playerToUpdate = this.players.find(eachPlayer => eachPlayer.id === targetPlayer.id);
      if (playerToUpdate && playerToUpdate.id !== this._ourPlayer?.id) {
        playerToUpdate._pokemon = targetPlayer.pokemon;
        this.emit('updatePokemon', playerToUpdate);
      }
    });
  ...
  public emitDisplayPokemonUpdate(newPokemon: Pokemon | undefined) {
    this._socket.emit('pokemonDisplay', newPokemon);
  }
